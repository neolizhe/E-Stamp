...
class Stamp:
    def __init__(self,  edge = 5,               # 图片边缘空白的距离
                        ...
                        img_wl_path = fig_path + "wl.png", # 纹理图路径
                        save_path = fig_path + "stamp.png" # 保存图片路径
                 ):
    ...
    def draw_rotated_text(self, image, angle, xy, r, word, fill, font_size, font_xratio, stroke_width, font_flip = False, *args, **kwargs):
        ...
        if is_Chinese(word):
            font = ImageFont.truetype("/mnt/c/Windows/Fonts/simsun.ttc", font_size, encoding="utf-8")
        else:
            font = ImageFont.truetype("/mnt/c/Windows/Fonts/arial.ttf", font_size, encoding="utf-8")
